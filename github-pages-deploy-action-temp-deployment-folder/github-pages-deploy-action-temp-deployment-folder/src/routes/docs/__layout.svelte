...
<Metadata title="Handyempfehlungen - Docs - {currentPage?.name}" description="" />
	...
	<div class="docs-container-inner">
		...
		<article class="markdown-body" bind:this={article}>
			<header>
				...
				<Button
					href="https://github.com/Niktendo/Home/edit/main/src/routes/docs{currentPage?.path ||
						'/index'}.md"
					rel="noreferrer noopener"
					target="_blank"
					variant="hyperlink">Diese Seite bearbeiten</Button
				>
			</header>
			<slot />
			{#if currentPage?.examples}
		...
		<aside>
			<TextBlock variant="bodyStrong">Inhalt</TextBlock>
			<Toc target={article} />
		</aside>
	</div>
</main>
